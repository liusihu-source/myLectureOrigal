...
var maxSubArray = function(nums) {
    ...
    let anyPlaceMax = nums[0]
    for(let i = 1; i < nums.length; i++){
        //全是负数情况
        if(currentSum < 0){
            currentSum = nums[i]
            anyPlaceMax = Math.max(anyPlaceMax, nums[i])
            continue
        }
        // 记录当前和
        currentSum = currentSum + nums[i]
        //始终保存最大值
        anyPlaceMax = Math.max(anyPlaceMax, currentSum)
        // 如果当前值和下一个加起来小于0,重新定义起点
        if(currentSum < 0){
            currentSum = nums[i]
        }
    }
...
var addTwoNumbers = function(l1, l2) {
    let target = new ListNode()  //提供的类
    let currentVal = target
    let currentNumber = 0
    while(l1 || l2 || currentNumber){
        if(l1){
        ...
        currentVal = currentVal.next = new ListNode(currentNumber % 10)
