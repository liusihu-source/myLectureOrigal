...
在Java中，声明方法时，当在完成方法功能时如果有`未知的数据`需要参与，这些未知的数据需要在调用方法时才能确定，那么我们把这样的数据通过`形参`表示。在方法体中，用这个形参名来代表那个未知的数据，而调用者在调用时，对应的传入`实参`就可以了。
...
![image-20231116172223243](泛型.assets/image-20231116172223243.png)
...
![image-20231122093955190](泛型.assets/image-20231122093955190.png)

**集合中使用泛型时：**

![image-20231122094007550](泛型.assets/image-20231122094007550.png)
...
（3）在测试类中，创建Rectangle数组，并创建4个矩形对象
...
> 2. 如果在创建自定义泛型类的对象时，没有指明泛型参数类型，那么泛型将被擦除，泛型对应的类型均按照Object处理，但不等价于Object。
>
>    - `经验：泛型要使用一路都用。要不用，一路都不要用`
> 3. 泛型的指定中必须使用引用数据类型。不能使用基本数据类型，此时只能使用包装类替换。
> 4. 如果我们在给泛型类提供子类时，子类也不确定泛型的类型，则可以继续使用泛型参数。我们还可以在现有的父类的泛型参数的基础上，新增泛型参数。
> 5.  泛型类可能有多个参数，此时应将多个参数一起放在尖括号内。比如：`<E1,E2,E3>`
> 6. `异常类不能是带泛型的`
...
定义个泛型类 DAO`<T>`，在其中定义一个Map 成员变量，Map 的键为 String 类型，值为 T 类型；并在该类中分别创建以下方法：
...
![image-20231122153529568](泛型.assets/image-20231122153529568.png)

### 3.3 自定义泛型方法

如果定义类、接口时没有使用<泛型参数>，但是某个方法形参类型不确定时，这个方法可以单独定义<泛型参数>。
