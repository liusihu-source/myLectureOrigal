...
![image-20230914161555611](面向对象高级.assets/image-20230914161555611.png)
...
![image-20230914161855649](面向对象高级.assets/image-20230914161855649.png)
...
public class StaticTest {
    public static void main(String[] args) {
        Demo test = null;
        test.hello();   //正常打印hello,不会报错，因为静态方法的调用看编译时类型
    }
}
...
![image-20230914205713554](面向对象高级.assets/image-20230914205713554.png)

#### 2.2 单例模式的概念

所谓类的单例设计模式，就是采取一定的方法保证在整个的软件系统中，对某个类`只能存在一个对象实例`，并且该类只提供`一个取得其对象实例`的方法。
...
:bulb: 获取系统当前时间 System.currentTimeMillis() //距离1970-1-1 00：00：00
...
如果想要为静态变量初始化，可以直接在静态变量的声明后面直接赋值（即显式赋值），也可以使用静态代码块。
...
public class Something {
    ...
    public void addOne(final Other o) {
        // o = new Other();  //更改o的地址值错误
        o.i++;  //o的地址值没变，只是变了内部值，没有错误
    }
}
class Other {
    public int i;
...
:bulb: final + static => 常量

## 五、抽象类和抽象方法（abstract关键字）
...
![image-20230917143758693](面向对象高级.assets/image-20230917143758693.png)
...
![image-20230917143930655](面向对象高级.assets/image-20230917143930655.png)
...
![image-20230918210454587](面向对象高级.assets/image-20230918210454587.png)
...
请输入当前的月份：
12
SalariedEmployee[name = 张小亮,number = 1001, birthday = 1992年12月21日]
工资为：18000.0
生日快乐！加薪100
HourlyEmployee[name = 侯少鹏,number = 1002, birthday = 1997年11月12日]
工资为：24000.0
```
...
![image-20230919103658776](面向对象高级.assets/image-20230919103658776.png)
...
public class InterfaceDemo{
	public static void main(String args[]){
		...
		c.show(new USB(){ //后面常用，不用单独新建实现类，这里直接创建匿名实现类的匿名对象
			public void start(){
...
![image-20230919105457897](面向对象高级.assets/image-20230919105457897.png)
...
实现类实现接口，类似于子类继承父类，因此，`接口类型的变量与实现类的对象之间，也可以构成多态引用。通过接口类型的变量调用方法，最终执行的是new的实现类对象实现的方法体`。
...
> 当一个类，既继承一个父类，又实现若干个接口时，父类中的成员方法与接口中的抽象方法重名，代码如下：
>
...
![image-20230919165503949](面向对象高级.assets/image-20230919165503949.png)
...
![image-20230919170333850](面向对象高级.assets/image-20230919170333850.png)

其中，有一个乘坐交通工具的方法takingVehicle()，在此方法中调用交通工具的run()。为了出行方便，他买了一辆捷安特自行车、一辆雅迪电动车和一辆奔驰轿车。这里涉及到的相关类及接口关系如下：

![image-20230919170339721](面向对象高级.assets/image-20230919170339721.png)

其中，电动车增加动力的方式是充电，轿车增加动力的方式是加油。在具体交通工具的run()中调用其所在类的相关属性信息。

请编写相关代码，并测试。

:bulb: 创建Vehicle[]数组，保存阿里工程师的三辆交通工具，并分别在工程师的takingVehicle()中调用。
...
![image-20230921111444533](面向对象高级.assets/image-20230921111444533.png)
...
举例2：通过`父类或父接口的变量多态`引用`匿名内部类的对象`
...
 
	public static void main(String[] args) {
		@SuppressWarnings("unused")   //去除没有使用的警告
		int a = 10;
	}
	@Deprecated  //过时警告
	public void print(){
...
![image-20230923111823431](面向对象高级.assets/image-20230923111823431.png)

![image-20230923111831307](面向对象高级.assets/image-20230923111831307.png)
...
![image-20230923113600512](面向对象高级.assets/image-20230923113600512.png)

**注意Scope：选择Compile，否则编译时，无法使用JUnit。**

第2步：下次如果有新的模块要使用该libs库，这样操作即可

![image-20230923113653435](面向对象高级.assets/image-20230923113653435.png)

![image-20230923113701194](面向对象高级.assets/image-20230923113701194.png)

##### 9.6.4 编写和运行@Test单元测试方法
...
![image-20230923113850171](面向对象高级.assets/image-20230923113850171.png)

![image-20230923113912121](面向对象高级.assets/image-20230923113912121.png)
...
![image-20230923114044784](面向对象高级.assets/image-20230923114044784.png)

![image-20230923114101183](面向对象高级.assets/image-20230923114101183.png)
...
Java针对`八种基本数据类型定义了相应的引用类型：包装类（封装类）`。有了类的特点，就可以调用类中的方法，Java才是真正的面向对象。

![image-20230923182331862](面向对象高级.assets/image-20230923182331862.png)
...
![image-20230923182357496](面向对象高级.assets/image-20230923182357496.png)
...
![image-20230923183431046](面向对象高级.assets/image-20230923183431046.png)
