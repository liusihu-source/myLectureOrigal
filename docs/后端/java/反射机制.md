...
![image-20231130104405728](反射机制.assets/image-20231130104405728.png)

**从内存加载上看反射：**

![image-20231130104504771](反射机制.assets/image-20231130104504771.png)
...
![image-20231130112657815](反射机制.assets/image-20231130112657815.png)
...
![image-20231130124817869](反射机制.assets/image-20231130124817869.png)

:bulb:` 上图中字符串常量池在JDK6中存储在方法区；JDK7及以后，存储在堆空间。`
...
![image-20231130125748229](反射机制.assets/image-20231130125748229.png)
...
![image-20231130132820553](反射机制.assets/image-20231130132820553.png)

### 3.2 类的加载过程

当程序主动使用某个类时，如果该类还未被加载到内存中，系统会通过加载、链接、初始化三个步骤来对该类进行初始化。如果没有意外，JVM将会连续完成这三个步骤，所以有时也把这三个步骤统称为类加载。

![image-20231130132829578](反射机制.assets/image-20231130132829578.png)
...
②准备Prepare :star: ：正式为类变量（static）分配内存并`设置类变量默认初始值`的阶段，这些内存都将在方法区中进行分配。
...
![image-20231130134217847](反射机制.assets/image-20231130134217847.png)
...
![image-20231130135047206](反射机制.assets/image-20231130135047206.png)

#### 3.3.2 类装载器的分类(JDK8为例)

JVM支持两种类型的类加载器，分别为`引导类加载器（Bootstrap ClassLoader）`和`自定义类加载器（User-Defined ClassLoader）`。

从概念上来讲，自定义类加载器一般指的是程序中由开发人员自定义的一类类加载器，但是Java虚拟机规范却没有这么定义，而是将所有派生于抽象类ClassLoader的类加载器都划分为自定义类加载器。无论类加载器的类型如何划分，在程序中我们最常见的类加载器结构主要是如下情况：

![image-20231130135957848](反射机制.assets/image-20231130135957848.png)
...
![image-20231130140006774](反射机制.assets/image-20231130140006774.png)
...
![image-20231130142759941](反射机制.assets/image-20231130142759941.png)
