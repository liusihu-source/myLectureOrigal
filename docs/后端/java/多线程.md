...
* **线程（thread）**：进程可进一步细化为线程，是程序内部的`一条执行路径`。一个进程中至少有一个线程。
  ...
  - 一个进程同一时间若`并行`执行多个线程，就是支持多线程的。
    ...
    ![image-20230927104218212](多线程.assets/image-20230927104218212.png)
...
![image-20230927104336369](多线程.assets/image-20230927104336369.png)
...
![image-20230927104427601](多线程.assets/image-20230927104427601.png)

2、一个应用程序的多次运行，就是多个进程

![image-20230927104442929](多线程.assets/image-20230927104442929.png)

3、一个进程中包含多个线程

![image-20230927104458885](多线程.assets/image-20230927104458885.png)
...
![image-20230930190005687](多线程.assets/image-20230930190005687.png)
...
![image-20230927105116494](多线程.assets/image-20230927105116494.png)

![image-20230927105121034](多线程.assets/image-20230927105121034.png)

- **并发（concurrency）**：指两个或多个事件在`同一个时间段内`发生。即在一段时间内，有`多条指令`在`单个CPU`上`快速轮换、交替`执行，使得在宏观上具有多个进程同时执行的效果。

  ![image-20230927105141376](多线程.assets/image-20230927105141376.png)

  ![image-20230927105148188](多线程.assets/image-20230927105148188.png)
...
:bulb: 说明：Runnable对象仅仅作为Thread对象的target，Runnable实现类里包含的run()方法仅作为线程执行体。
...
![image-20231007183201654](多线程.assets/image-20231007183201654.png)
...
![image-20231007183328772](多线程.assets/image-20231007183328772.png)

或

![image-20231007183338745](多线程.assets/image-20231007183338745.png)
...
![image-20231007183416972](多线程.assets/image-20231007183416972.png)
...
![image-20231009180453045](多线程.assets/image-20231009180453045.png)

### 5.1 同一个资源问题和线程安全问题

案例：

火车站要卖票，我们模拟火车站的卖票过程。因为疫情期间，本次列车的座位共100个（即，只能出售100张火车票）。模拟车站的售票窗口，实现多个窗口同时售票的过程。注意：不能出现错票、重票。
...
![image-20231009181529118](多线程.assets/image-20231009181529118.png)
...
![image-20231011121647445](多线程.assets/image-20231011121647445.png)

:warning: 一旦出现死锁，整个程序既不会发生异常，也不会给出任何提示，只是所有线程处于阻塞状态，无法继续。
...
1. wait方法与notify方法必须要由`同一个锁对象调用`
...
![image-20231016161333269](多线程.assets/image-20231016161333269.png)
