...
![image-20231129092510193](网络编程.assets/image-20231129092510193.png)

- **B/S架构** ：全称为Browser/Server结构，是指浏览器和服务器结构。常见浏览器有IE、谷歌、火狐等。

![image-20231129092517236](网络编程.assets/image-20231129092517236.png)
...
![image-20231129092721624](网络编程.assets/image-20231129092721624.png)
...
> 在一般情况下，每台连接到互联网的设备都有一个独特的 IP 地址，这个 IP 地址用于在网络上唯一标识这台设备。IP 地址是网络通信中非常重要的概念，它类似于现实生活中的邮政地址，用于定位和标识网络上的设备。
>
> 然而，有一些情况下并非每台设备都会有自己的公共 IP 地址：
>
> - **IPv4地址短缺**：由于IPv4地址空间的限制，一些设备可能会共享一个公共IP地址，这是通过路由器和NAT(Network Address Translation)技术实现的。在这种情况下，局域网中的多台设备共享一个公共 IP 地址，但在互联网上仍然可以唯一标识这些设备，因为它们会被分配不同的局域网内部IP地址。
> - **IPv6地址**：IPv6 地址空间非常庞大，几乎每台设备都可以拥有一个独特的全球唯一的IPv6地址。然而，目前在全球范围内还在逐步过渡到IPv6，很多设备仍然使用IPv4。

**IP地址分类方式一：**

- `IPv4`：是一个32位的二进制数，通常被分为4个字节，表示成`a.b.c.d` 的形式，以点分`十进制`表示，例如`192.168.65.100` 。其中a、b、c、d都是0~255之间的十进制整数。

  ![image-20231129094049738](网络编程.assets/image-20231129094049738.png)
...
- `IPv6`：由于互联网的蓬勃发展，IP地址的需求量愈来愈大，但是网络地址资源有限，使得IP的分配越发紧张。
  ...
  > 十六进制是一种基数为16的数制系统，它使用0-9的数字和A-F（或a-f）的字母来表示数值。每一位的位置权值是16的幂次方。
  >
  > ### 表示方式：
  >
  > - **0-9：** 表示十进制的0到9。
  > - **A-F（或a-f）：** 分别表示十进制的10到15。
  >
  > ### 举例：
  >
  > - **十进制到十六进制的转换：**
...
![image-20231129101304265](网络编程.assets/image-20231129101304265.png)

详细理解：

![image-20231129105624073](网络编程.assets/image-20231129105624073.png)

1. 在浏览器中输入www . qq .com 域名，操作系统会先检查自己本地的`hosts文件`是否有这个网址映射关系，如果有，就先调用这个IP地址映射，完成域名解析。

   ![image-20231129105705407](网络编程.assets/image-20231129105705407.png)
...
2. 如果hosts里没有这个域名的映射，则查找`本地DNS解析器缓存`，是否有这个网址映射关系，如果有，直接返回，完成域名解析。
   ...
   ![image-20231129110044410](网络编程.assets/image-20231129110044410.png)
...
![image-20231129110906455](网络编程.assets/image-20231129110906455.png)
...
![image-20231129111327233](网络编程.assets/image-20231129111327233.png)
...
![image-20231129113418986](网络编程.assets/image-20231129113418986.png)

**TCP/IP协议中的四层介绍：**

- `应用层`：应用层决定了向用户提供应用服务时通信的活动。主要协议有：HTTP协议、FTP协议、SNMP（简单网络管理协议）、SMTP（简单邮件传输协议）和POP3（Post Office Protocol 3的简称,即邮局协议的第3个版）等。
- `传输层`：主要使网络程序进行通信，在进行网络通信时，可以采用TCP协议，也可以采用UDP协议。TCP（Transmission Control Protocol）协议，即传输控制协议，是一种面向连接的、可靠的、基于`字节流`的传输层通信协议。UDP(User Datagram Protocol，用户数据报协议)：是一个无连接的传输层协议、提供面向事务的简单不可靠的信息传送服务。
...
![image-20231129122501901](网络编程.assets/image-20231129122501901.png)
...
![image-20231129143036157](网络编程.assets/image-20231129143036157.png)
...
![image-20231129143303089](网络编程.assets/image-20231129143303089.png)
...
![image-20231129173402106](网络编程.assets/image-20231129173402106.png)
...
![image-20231129195920288](网络编程.assets/image-20231129195920288.png)
...
![image-20231129203858240](网络编程.assets/image-20231129203858240.png)
...
![image-20231129210649670](网络编程.assets/image-20231129210649670.png)
