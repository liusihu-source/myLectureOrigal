...
![image-20231127112401132](File类与IO流.assets/image-20231127112401132.png)
...
练习1：判断指定目录下是否有后缀名为.jpg的文件。如果有，就输出该文件名称
...
![image-20231127135829049](File类与IO流.assets/image-20231127135829049.png)
...
![image-20231127143617842](File类与IO流.assets/image-20231127143617842.png)

### 2.1 Java IO原理

- Java程序中，对于数据的输入/输出操作以“`流(stream)`” 的方式进行，可以看做是一种数据的流动。

![image-20231127144017698](File类与IO流.assets/image-20231127144017698.png)
...
![image-20231127144154850](File类与IO流.assets/image-20231127144154850.png)
...
- 根据IO流的角色不同分为：**节点流**和**处理流**。
  ...
  ![image-20231127144848394](File类与IO流.assets/image-20231127144848394.png)

  - **处理流**：不直接连接到数据源或目的地，而是“连接”在已存在的流（节点流或处理流）之上，通过对数据的处理为程序提供更为强大的读写功能。

    ![image-20231127144929857](File类与IO流.assets/image-20231127144929857.png)

小结：图解

![image-20231127144941629](File类与IO流.assets/image-20231127144941629.png)
...
![image-20231127151957336](File类与IO流.assets/image-20231127151957336.png)
...
- `public int read()`： 从输入流读取一个字符。 虽然读取了一个字符，但是会自动提升为int类型。返回该字符的Unicode编码值。如果已经到达流末尾了，则返回-1。
  - :bulb: Unicode值：一种字符编码标准。Unicode 编码的范围从 `U+0000` 到 `U+10FFFF`，总共有超过一百万个字符位置。其中，前 128 个字符与 ASCII 编码一致，也就是说 ASCII 编码是 Unicode 的一个子集。Unicode 的不断更新和扩展使得它成为了现代计算机系统中使用的主要字符编码方案之一。
  - Unicode 编码通常用 `\u` 开头的十六进制数字表示一个字符的编码值。例如，`A` 字符的 Unicode 编码是 `U+0041`，可以用 `\u0041` 表示。
    - `A` 的 Unicode 编码是 `\u0041`
    - `中` 的 Unicode 编码是 `\u4E2D`
    - `😊`（笑脸表情）的 Unicode 编码是 `\uD83D\uDE0A`
...
> :warning: 注意：当完成流的操作时，必须调用close()方法，释放系统资源，否则会造成内存泄漏。
...
![image-20231127185739835](File类与IO流.assets/image-20231127185739835.png)
...
- `public int read(byte[] b)`： 从输入流中读取一些字节数，并将它们存储到字节数组 b中 。每次最多读取b.length个字。返回实际读取的字节个数。如果已经到达流末尾，没有数据可读，则返回-1。 
...
![image-20231127222816822](File类与IO流.assets/image-20231127222816822.png)

![image-20231127222821151](File类与IO流.assets/image-20231127222821151.png)
...
![image-20231128101536752](File类与IO流.assets/image-20231128101536752.png)
...
把当前module下的《康师傅的话.txt》字符编码为GBK，复制到电脑桌面目录下的《寄语.txt》，字符编码为UTF-8。在当前module下的文本内容：
...
![image-20231128163623766](File类与IO流.assets/image-20231128163623766.png)
...
- public void writeUTF(String str)：将给定的字符串按照 UTF-8 编码写入输出流。
...
![image-20231128195440914](File类与IO流.assets/image-20231128195440914.png)
...
![image-20231128200755131](File类与IO流.assets/image-20231128200755131.png)
...
![image-20231128203917020](File类与IO流.assets/image-20231128203917020.png)
...
- 打印流：`PrintStream`和`PrintWriter`
  ...
  - 提供了一系列重载的print()和println()方法，用于多种数据类型的输出
    ...
    ![image-20231128211315580](File类与IO流.assets/image-20231128211315580.png)

    ![image-20231128211433916](File类与IO流.assets/image-20231128211433916.png)
...
![image-20231128212444786](File类与IO流.assets/image-20231128212444786.png)
